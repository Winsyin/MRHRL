...
def MyLabel_val_test(valSample, testSample):
	label = []
	for i in range(int(len(valSample) / 2)):
		label.append(1)
	for i in range(int(len(testSample) / 2)):
		label.append(1)
	for i in range(int(len(valSample) / 2)):
		label.append(0)
	for i in range(int(len(testSample) / 2)):
...
if __name__=='__main__':
	...
	for fold in range(5):
		...
		y_val_test = MyLabel_val_test(x_validation_pair, x_test_pair)

		CircEmbeddingFeature = np.loadtxt('circRNA_seq_similarity_circRNA2vec.txt', delimiter='\t')
		miRNAEmbeddingFeature = np.loadtxt('miRNA_seq_similarity_kmer.txt', delimiter='\t')
		...
		hy_cm, hy_mc, hy_cc, hy_mm, circRNA_mat, miRNA_mat = load_hyg('./hyg/hyg_dict{}.pickle'.format(fold))
		...
		for e in range(epochs):
			...
			ModelTestOutput = test(CircEmbeddingFeature, miRNAEmbeddingFeature,
							x_val_test_pair, np.array(y_val_test), [hy_cm, hy_mc, hy_cc, hy_mm])
